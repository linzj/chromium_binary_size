...
#if UC_BUILD_DEBUG_SUPPORT
    self._wait_for_debugger = test_instance.wait_for_debugger
    # gtest has two step to run instrumentation. the first step is
    # to list test case, and the secode is to run test case. so we
    # should enbale debug flag in the second step
    self._enable_debug_flag = False
#endif

  #if UC_BUILD_DEBUG_SUPPORT
  def EnableDebugFlag(self, enable):
    self._enable_debug_flag = enable;

  def HasWaitForDebuggerFlag(self):
...
#if UC_BUILD_DEBUG_SUPPORT
    if self._enable_debug_flag and self._wait_for_debugger:
      if not flags:
        flags = '--wait-for-debugger'
      else:
        flags = flags + ' --wait-for-debugger'
#endif
  ...
  def _RunTest(self, device, test):
    ...
    with contextlib_ext.Optional(
      ...
      with device_temp_file.DeviceTempFile(
        ...
        #if UC_BUILD_DEBUG_SUPPORT
        if self._delegate.HasWaitForDebuggerFlag():
          self._delegate.EnableDebugFlag(True)
          print ""
          print "**********************************************************"
          print "******run: uc/build/debug_gtests.sh to debug program******"
          print "**********************************************************"
          print ""
          print timeout
        #endif
        ...
        #if UC_BUILD_DEBUG_SUPPORT
        if self._delegate.HasWaitForDebuggerFlag():
          self._delegate.EnableDebugFlag(False)
        #endif
